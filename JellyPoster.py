...
#JellyPoster Autheticate to Jellyfin and Get Headers
def do_AUTH():
    #Jellyfin Authetication
    jellyfin_auth_data = {
        "Username": jellyfin_username,
        "Pw": jellyfin_password,
    }

    jellyfin_authorization = (
        'MediaBrowser , '
        'Client="JellyPoster", '
        'Device="script", '
        'DeviceId="script", '
    ...
    #Jellyfin Headers
    jellyfin_headers = {}
    jellyfin_headers['x-emby-authorization'] = jellyfin_authorization

    #Log in and get userid and token
    jellyfin_login = requests.post(f'{jellyfin_url}/Users/AuthenticateByName', headers=jellyfin_headers, json=jellyfin_auth_data)
    jellyfin_token = jellyfin_login.json().get('AccessToken')
    jellyfin_userid = jellyfin_login.json().get('User').get('Id')

    jellyfin_headers['x-mediabrowser-token'] = jellyfin_token
    return jellyfin_headers, jellyfin_userid
...
class JellyPosterHTTP(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if http_path == "/":
            #Check for Playing
            jellyfin_client_match = check_PLAYING(jellyfin_headers)
            if jellyfin_client_match == "true":
                jellyposter_top, jellyposter_middle, jellyposter_bottom = get_PLAYING(jellyfin_headers)
            else:
                jellyposter_top, jellyposter_middle, jellyposter_bottom = get_RANDOM(jellyfin_headers)
...
def check_PLAYING(jellyfin_headers):
    jellyfin_client_match = "false"

    playing_devices = requests.get(f'{jellyfin_url}/Sessions?ActiveWithinSeconds=90', headers=jellyfin_headers)
...
def get_PLAYING(jellyfin_headers):
    playing_devices = requests.get(f'{jellyfin_url}/Sessions?ActiveWithinSeconds=90', headers=jellyfin_headers)
    for playing_device in playing_devices.json():
        if playing_device.get('NowPlayingItem'):
...
def get_RANDOM(jellyfin_headers):
    get_random_movie_poster = requests.get(f'{jellyfin_url}/Items?Userid={jellyfin_userid}&limit=1&Recursive=true&IncludeItemTypes=Movie&IsPlayed=False&sortBy=Random&api_key={jellyfin_api_key}', headers=jellyfin_headers)
...
jellyfin_headers, jellyfin_userid = do_AUTH()
